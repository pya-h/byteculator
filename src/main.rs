...
fn rem_redundant(mut str_num: String) -> String{
  let fst_digit_idx = if str_num.chars().nth(0) == Some('-') { 1 } else { 0 };
...
fn add(op1: String, op2: String) -> String {
  ...
  r = rem_redundant(r);
  if fstop_negative && sndop_negative {format!("-{}", r)} else {r}
}
...
fn mul(op1: String, op2: String) -> String {
  ...
  let fstop_sign: i8 = if fstop.chars().nth(0) == Some('-') { -1 } else {1};
  let sndop_sign: i8 = if sndop.chars().nth(0) == Some('-') { -1 } else {1};
  ...
  if fstop_sign * sndop_sign == -1 { format!("-{}", r) } else { r }
}
...
fn pow(base: String, power: String, log: bool) -> String {
  ...
  if log {
    println!("\n{}--------------------------------- {} -----------------------------", PADDING, format!("{} ^ {}", base, power).yellow().italic());
  ...
  for i in 0..temp_power {
    ...
    if log {
      println!("{} = {}{}", if length_long {"\n\t"} else { "" }, format!("{}", result).bold().blue(), if length_long {"\n"} else { "" });
    }
  }

  if log {
    println!("\n{}--------------------------- {} -----------------------------", PADDING, format!("END OF {} ^ {}", base, power).yellow().italic());
